...
3. Load `references/engineering-standards.md` (once per feature - its completion checklist gates every slice) and the matching domain skills - `spring-api` (endpoints), `migrations` (DDL), `ledger` (money movement), `temporal` (workflows), `igaming-ui` (betting/casino UI) - plus repo CLAUDE.md. Detect the toolchain from the repo (Java, Boot, and build-tool versions differ across workspaces) - never assume.
