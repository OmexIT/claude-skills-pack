...
3. Load the matching domain skills — `spring-api` (endpoints), `migrations` (DDL), `ledger` (money movement), `temporal` (workflows) — plus repo CLAUDE.md. Detect stack versions from the repo (employer: Java 21/Boot 3/Maven; personal: Java 25/Boot 4/Gradle) — never assume.
