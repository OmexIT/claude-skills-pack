...
This skill generates a multi-file resource stack. Treat the work as a planned implementation, not a one-shot scaffold.

1. Clarify the resource shape before generation: fields, validation, error cases, pagination strategy, status codes, and idempotency requirements. Skip only when the OpenAPI fragment is already comprehensive.
2. Produce a short file-and-test plan listing every generated file, validation rule, and test case before touching code.
3. Prefer an isolated branch or worktree when controller, service, repository, mapper, and migration changes could collide with other feature work.
4. Write the `@WebMvcTest` slice test first (201 on create, 400 on invalid, 404 on missing, idempotency-key replay), then the integration test with Testcontainers, then implement.
5. Verify before claiming done: compile (`./gradlew compileJava` or `./mvnw compile`), run slice tests, run integration tests, and hit the live endpoint with `curl` to confirm the envelope response.
6. Before merge, review error shape (RFC 9457), pagination cursor correctness, mapper null handling, and OpenAPI spec parity.
