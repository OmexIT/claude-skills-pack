...
Structured test-plan format used by `e2e` for features that warrant one.

Written before verification begins. Saved to: `e2e/test-plan.yaml`
  ...
  - id:        TC-001
    ...
    api:
      - step:          1
        ...
        expect:
          status:      201
          body_fields: ["data.id", "data.status", "data.amount"]  # dot-paths that must exist
    ...
    db:
      ...
      - check:         "ledger_balanced"       # fintech-specific; must mirror the zero-row
        description:   "Double-entry ledger balanced"  # query in ledger's pgledger-invariants.md
        table:         "ledger_postings"
        transaction_id_field: "transaction_id"
        transaction_id_value: "{resource_id}"
        direction_field: "direction"           # DEBIT | CREDIT
        amount_field:  "amount"
...
<project-root>/
└── e2e/
    ├── test-plan.yaml          ← test plan (produced and consumed by `e2e`)
    └── screenshots/            ← screenshot evidence from UI runs
```
