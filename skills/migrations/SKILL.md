...
- **Audit quartet** on every table: `created_at`, `created_by`, `updated_at`, `updated_by` (`TIMESTAMPTZ`).
...
## Backfills
Batched `DO` loop with `pg_sleep` throttle, `--runInTransaction:false`; when rollback is `SELECT 1`, mark the changeset irreversible and say so in the plan/PR. Pattern: `references/backfill-pattern.md`.
